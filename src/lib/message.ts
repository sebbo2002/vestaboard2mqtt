import {
    ...
    emptyBoard,
    LINE_LENGTH,
} from 'vestaboard-api/lib/cjs/values.js';
...
export interface MessageWriteOptions {
    line?: MessageWriteOptionsLine | number;
    row?: number;
}
...
export default class Message {
    ...
    private currentLine: null | number = null;
    ...
    static char2char(char: string): number[] {
        const fromVestaMap = Object.entries(characterCode).find(
            ([key]) => char === key,
        );
        if (fromVestaMap) {
        ...
        const fromMyMap = SPECIAL_CHAR_MAP.find(
            ([mapChar]) => char === mapChar,
        );
    ...
    center(): void {
        ...
        if (space[0] !== padding[0]) {
            ...
            this.board.splice(
                space[0] < padding[0] ? 0 : this.board.length - add,
                0,
        ...
        if (space[3] !== padding[1]) {
            ...
            this.board.forEach((line) => {
                line.splice(
                    space[3] < padding[1] ? 0 : line.length - add,
                    0,
    ...
    write(text: string, options: MessageWriteOptions = {}): void {
        ...
        words.forEach((word, i) => {
            ...
            if (
                chars.filter((c) => c === 60).length === chars.length &&
                chars.length > 0
            ) {
                return;
            }
