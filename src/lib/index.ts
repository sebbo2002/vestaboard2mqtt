...
export default class Vestaboard2MQTT {
    ...
    constructor() {
        ...
        this.mqtt.on('connect', () => {
            ...
            this.setupBrokerSubscriptions().catch((error) => {
                console.error(
                    new Error(
                        `Unable to setup mqtt subscriptions: ${error.stack}`,
                    ),
                );
                process.exit(1);
            });
    ...
    private async handleMessage(topic: string, payload: string): Promise<void> {
        if (
            [
                Config.mqtt.prefix + '/debug',
                Config.mqtt.prefix + '/status',
            ].includes(topic)
        ) {
            ...
            if (!value) {
                this.debug(
                    'Board not disabled anymore, update with latest message',
                );
                await this.updateCurrentMessage();
            }
        } else if (
            Object.keys(pages)
                .map((id) => Config.mqtt.prefix + '/' + id)
                .includes(topic)
        ) {
            const page = pages[topic.substr(Config.mqtt.prefix.length + 1)];
            await this.renderMessage(page, payload);
        } else {
            const page = Object.entries(pages).find(([id]) =>
                topic.startsWith(Config.mqtt.prefix + '/' + id + '/'),
            );
            ...
            const key = topic.substr(
                (Config.mqtt.prefix + '/' + page[0] + '/').length,
            );
            ...
            await this.updateCurrentMessage();
            this.debug(
                `Update config for page module ${page[0]}: ${JSON.stringify(newConfig, null, '    ')}`,
            );
        }
    }
    ...
    private async renderMessage(page: Page<unknown>, payload: string) {
        ...
        const pageEntry = Object.entries(pages).find(
            ([, pageInstance]) => pageInstance === page,
        );
        ...
        const config = await this.pageConfig.get<Partial<unknown>>(
            pageEntry[0],
        );
        ...
        if (response.validTill) {
            this.debug(
                `Set timer for ${response.validTill.toString()} to update message`,
            );
    ...
    private async sendMessage(message: Message = new Message()): Promise<void> {
        ...
        await Promise.all(
            this.subscriptionIds.map((id) =>
                this.board.postMessage(id, charArray),
            ),
        );
    }
